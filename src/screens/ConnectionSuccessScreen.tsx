...
export const ConnectionSuccessScreen: React.FC = () => {
  ...
  const quickMessageOptions = useMemo(
    () => [
      t('buddy.connectionSuccess.quickActions.templates.sayHi1', { name: sender.display_name }),
      t('buddy.connectionSuccess.quickActions.templates.sayHi2', { name: sender.display_name }),
      t('buddy.connectionSuccess.quickActions.templates.sayHi3', { name: sender.display_name }),
    ],
  ...
  const ensureChat = useCallback(async () => {
    if (!currentUserId) {
      throw new Error(t('buddy.connectionSuccess.errors.missingUser'));
    }
    const response = await createDirectChat(currentUserId, sender.user_id);
    if (!response.success || !response.chat) {
      if (response.errorCode === 'PERMISSION_DENIED') {
        throw new Error(t('buddy.connectionSuccess.errors.chatPermission'));
      }
      throw new Error(response.error || t('buddy.connectionSuccess.errors.chatFailed'));
    }
  ...
  const handlePrimaryAction = useCallback(async () => {
    try {
      ...
      showSuccessToast(t('buddy.connectionSuccess.actions.chatReady'));
      ...
      const message =
        error instanceof Error ? error.message : t('buddy.connectionSuccess.errors.chatFailed');
  ...
  const handleQuickMessage = useCallback(
    async (message: string) => {
      try {
        ...
        showSuccessToast(t('buddy.connectionSuccess.quickActions.sent'));
      } catch (error) {
        const message =
          error instanceof Error ? error.message : t('buddy.connectionSuccess.errors.chatFailed');
        showErrorToast(message);
  ...
  const handleStudySession = useCallback(() => {
    showErrorToast(t('buddy.connectionSuccess.quickActions.comingSoon'));
  }, [t]);
          ...
          <View>
            ...
            >
              <BackButton
                ...
                accessibilityLabel={t('buddy.requests.goBack')}
                style={{
                  position: 'absolute',
                  top: theme.spacing[4],
              ...
              <View style={{ alignItems: 'center', marginTop: theme.spacing[4] }}>
                ...
                >
                  <Animated.View style={[pulseStyle, { position: 'absolute', left: 0, zIndex: 2 }]}>
                    <Avatar
                      size="lg"
                      name={currentUserProfile?.display_name ?? t('buddy.connectionSuccess.me')}
                ...
                <Text variant="body" color="secondary" style={{ opacity: 0.7 }}>
                  {t('buddy.connectionSuccess.headerTitle', { name: sender.display_name })}
                </Text>
              </View>
            </LinearGradient>
          </View>
            ...
            <>
              ...
              >
                <Text variant="h3" color="primary" style={{ fontWeight: '700' }}>
                  {t('buddy.connectionSuccess.title')}
                </Text>
                <Spacer size={2} />
                <Text variant="body" color="secondary">
                  {t('buddy.connectionSuccess.subtitle', { name: sender.display_name })}
                </Text>
              </Animated.View>
                ...
                >
                  ...
                  >
                    ...
                    <Text variant="h5" color="primary" style={{ fontWeight: '600' }}>
                      {t('buddy.connectionSuccess.matchBadge', { percentage: matchPercentage })}
                    </Text>
                  </View>
                </Animated.View>
              )}
              ...
              {loading ? (
                ...
                <Animated.View entering={FadeInDown.delay(600).duration(500)}>
                  <Text variant="body" color="secondary" style={{ textAlign: 'center' }}>
                    {t('buddy.connectionSuccess.noCommonPoints')}
                  </Text>
                </Animated.View>
              ) : (
                <Animated.View entering={FadeInDown.delay(600).duration(500)}>
                  ...
                  >
                    {t('buddy.connectionSuccess.commonPoints')}
                  </Text>
              ...
              >
                ...
                >
                  {t('buddy.connectionSuccess.quickActions.title')}
                </Text>
                ...
                >
                  ...
                  >
                    ...
                    <Text variant="body" color="primary" style={{ fontWeight: '600' }}>
                      {t('buddy.connectionSuccess.quickActions.sayHi.title')}
                    </Text>
                  </View>
                ...
                {showAchievement && (
                  ...
                  >
                    ...
                    <Text variant="bodySmall" color="primary" style={{ fontWeight: '600' }}>
                      {t('buddy.connectionSuccess.achievement.firstMatch')}
                    </Text>
                  </Animated.View>
                )}
                ...
                >
                  ...
                  <View style={{ flex: 1 }}>
                    <Text variant="body" color="primary" style={{ fontWeight: '600' }}>
                      {t('buddy.connectionSuccess.quickActions.createStudySession.title')}
                    </Text>
                    <Text variant="bodySmall" color="secondary">
                      {t('buddy.connectionSuccess.quickActions.createStudySession.description')}
                    </Text>
                  </View>
                </Pressable>
                ...
                >
                  ...
                  <View style={{ flex: 1 }}>
                    <Text variant="body" color="primary" style={{ fontWeight: '600' }}>
                      {t('buddy.connectionSuccess.quickActions.shareMaterial.title')}
                    </Text>
                    <Text variant="bodySmall" color="secondary">
                      {t('buddy.connectionSuccess.quickActions.shareMaterial.description')}
                    </Text>
                  </View>
                </Pressable>
              </Animated.View>
              ...
              <Animated.View entering={FadeInDown.delay(1000).duration(500)}>
                  ...
                  {chatLoading ? (
                    ...
                    <>
                      ...
                      <Text variant="h5" color="inverse" style={{ fontWeight: '600' }}>
                        {t('buddy.connectionSuccess.actions.chat', { name: sender.display_name })}
                      </Text>
                    </>
                  )}
                </Pressable>
                ...
                >
                  <Text variant="body" color="primary" style={{ fontWeight: '500' }}>
                    {t('buddy.connectionSuccess.actions.keepDiscovering')}
                  </Text>
                </Pressable>
              </Animated.View>
