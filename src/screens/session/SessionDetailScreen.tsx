...
export const SessionDetailScreen: React.FC = () => {
  ...
  const handleAcceptInvitation = async () => {
    ...
    try {
      await acceptInvitation(sessionId, userId);
      showSuccessToast('Đã chấp nhận lời mời tham gia buổi học');
      // Navigate back to home or notifications
      const parent = navigation.getParent();
      if (parent) {
      ...
      logger.error('SessionDetailScreen', 'Accept invitation error:', error);
    }
  };
  ...
  const handleDeclineInvitation = async () => {
    if (!userId || !sessionId) return;
    try {
      await declineInvitation(sessionId, userId);
      showSuccessToast('Đã từ chối lời mời tham gia buổi học');
      // Update local session state to reflect declined status in detail immediately
      setSession((prev) => {
        if (!prev) return prev;
        return {
          ...prev,
      ...
      logger.error('SessionDetailScreen', 'Decline invitation error:', error);
    }
  };
    ...
    Alert.alert('Hủy buổi học', 'Bạn có chắc chắn muốn hủy buổi học này?', [
      { text: 'Không', style: 'cancel' },
      {
        ...
        onPress: async () => {
          ...
          // Navigate to Home immediately after cancellation
          const parent = navigation.getParent();
          if (parent) {
            parent.navigate('MainTabs', { screen: 'Home' });
          } else {
  ...
  const currentUserParticipant = session.participants.find((p) => p.user_id === userId);
    ...
    <View style={{ flex: 1, backgroundColor: theme.colors.background }}>
      ...
      >
        <View style={{ paddingHorizontal: theme.spacing[5] }}>
              ...
              >
                ...
                {participant.status === 'declined' && (
                  <View
                    style={[
                      styles.statusBadge,
                      { backgroundColor: '#FFE4E6', paddingVertical: 4, paddingHorizontal: 12 },
                    ]}
                  >
          ...
          {/* Bottom Actions for Invited Users */}
          {readOnly && isInvited && (
            <View
              style={{
                flexDirection: 'row',
          ...
          {/* Bottom Actions - Only show for creator and if session is scheduled and not read-only */}
