...
export const BuddyRequestsScreen: React.FC = () => {
  ...
  const handleSwipeLeft = useCallback(
    (card: BuddyCardData) => {
      ...
      if (!request) {
        showErrorToast(t('buddy.request.invalidRequest'));
        return;
      }
      ...
      dispatch(respondToBuddyRequestAsync({ connectionId: request.id, action: 'reject' }))
        .unwrap()
        .then(() => {
          showInfoToast(t('buddy.request.rejected'));
        })
        .catch((error: { error?: string }) => {
          showErrorToast(error.error || t('buddy.request.rejectError'));
        });
    },
  ...
  const handleFilterPress = useCallback(() => {
    showInfoToast(t('buddy.requests.filterComingSoon'));
  }, [t]);
        ...
        >
          ...
          <View style={{ marginBottom: theme.spacing[4] }}>
            <Text variant="h6" color="primary" style={{ fontWeight: '600' }}>
              {t('buddy.requests.subtitle')}
            </Text>
            <Spacer size={1} />
            <Text variant="body" color="secondary">
              {t('buddy.requests.helperDescription')}
            </Text>
          </View>
          ...
          <View style={{ flex: 1 }}>
            {loading && incomingRequests.length === 0 ? (
              ...
              <EmptyState
                title={t('buddy.requests.noRequests')}
                description={
                  searchQuery
                    ? t('buddy.requests.noSearchResults', { keyword: searchQuery })
                    : t('buddy.requests.noRequestsMessage')
                }
                actionLabel={t('buddy.requests.goBack')}
                onActionPress={() => {
                  if (navigation.canGoBack()) {
                    navigation.goBack();
                  }
